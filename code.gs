...
function doPost(e) {
  ...
  try {
    ...
    if (action === "add_discount") {
      sheet = getOrCreateSheet("discounts", ["title", "description", "discount_value", "banner_url", "target_type", "target_id", "date_added", "active"]);
      var bannerUrl = "";
      if (data.imageFile) {
        try   { bannerUrl = uploadToGitHub(data.imageFile.name, data.imageFile.content, cfg); }
        catch (err) {
          console.error("Discount banner upload: " + err);
          uploadWarning = err.toString();
        }
      }
      sheet.appendRow([data.title, data.description, data.discount_value, bannerUrl, data.target_type, data.target_id, new Date(), data.active || "TRUE"]);
      return jsonOut({ result: "success", message: "Discount Added", upload_warning: uploadWarning });
    }
    ...
    if (action === "delete_discount") {
      ...
      rowIndex = parseInt(data.rowId, 10);
      sheet.deleteRow(rowIndex);
      return jsonOut({ result: "success", message: "Discount Deleted" });
    }

    if (action === "edit_discount") {
      sheet    = getOrCreateSheet("discounts", ["title", "description", "discount_value", "banner_url", "target_type", "target_id", "date_added", "active"]);
      rowIndex = parseInt(data.rowId, 10);
      var bannerUrl = data.existingBannerUrl || "";
      if (data.imageFile) {
        try   { bannerUrl = uploadToGitHub(data.imageFile.name, data.imageFile.content, cfg); }
        catch (err) {
          console.error("Discount banner upload (edit): " + err);
          uploadWarning = err.toString();
        }
      }
      sheet.getRange(rowIndex, 1, 1, 8).setValues([[data.title, data.description, data.discount_value, bannerUrl, data.target_type, data.target_id, new Date(), data.active || "TRUE"]]);
      return jsonOut({ result: "success", message: "Discount Updated", upload_warning: uploadWarning });
    }
    ...
    if (action === "edit") {
      ...
      sheet.getRange(rowIndex, 1, 1, 8).setValues([[
        ...
        data.description, finalImagesStr, new Date(), data.product_link || ""
      ]]);
      return jsonOut({ result: "success", message: "Product Updated", upload_warning: uploadWarning });
    }

    sheet.appendRow([data.name, data.category, data.price, data.old_price, data.description, finalImagesStr, new Date(), data.product_link || ""]);
...
function setupSheet() {
  getOrCreateSheet("products",     ["name", "category", "price", "old_price", "description", "images", "date_added", "product_link"]);
  getOrCreateSheet("brands",       ["name", "logo_url", "date_added"]);
  getOrCreateSheet("testimonials", ["name", "role", "text", "rating", "photo_url", "date_added"]);
  getOrCreateSheet("discounts",    ["title", "description", "discount_value", "banner_url", "target_type", "target_id", "date_added", "active"]);
}
